	...
	for(int i = 0; i < this->nodes; i++){
		for(int j = 0; j < this->nodes; j++){
			for(int k = 0; k < this->nodes; k++){
				if(adjacency_matrix_copy[j][k] > adjacency_matrix_copy[j][i] + adjacency_matrix_copy[i][k]){
					adjacency_matrix_copy[j][k] = adjacency_matrix_copy[j][i] + adjacency_matrix_copy[i][k];
					next[j][k] = next[j][i];
				}
			}
		}
